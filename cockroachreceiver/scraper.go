...
func (s *cockroachScraper) Start(ctx context.Context, host component.Host) error {
	...
	s.logger.Info("Successfully connected to CockroachDB",
		zap.Int("max_open_conns", s.config.MaxOpenConnections),
		zap.Int("max_idle_conns", s.config.MaxIdleConnections),
		zap.Duration("conn_max_lifetime", s.config.ConnectionMaxLifetime),
		zap.Duration("conn_max_idle_time", s.config.ConnectionMaxIdleTime),
...
func (s *cockroachScraper) ScrapeMetrics(ctx context.Context) (pmetric.Metrics, error) {
	...
	type scrape struct {
		enabled bool
		name    string
		fn      func(context.Context, *sql.DB, pmetric.ScopeMetrics, pcommon.Timestamp) error
	}
	scrapes := []scrape{
		{m.StatementStatistics, "statement_statistics", s.scrapeStatementStats},
		{m.TransactionStatistics, "transaction_statistics", s.scrapeTransactionStats},
		{m.IndexUsageStatistics, "index_usage_statistics", s.scrapeIndexUsage},
		{m.ClusterQueries, "cluster_queries", s.scrapeClusterQueries},
		{m.ClusterSessions, "cluster_sessions", s.scrapeClusterSessions},
		{m.ClusterTransactions, "cluster_transactions", s.scrapeClusterTransactions},
		{m.ClusterContendedIndexes, "cluster_contended_indexes", s.scrapeContendedIndexes},
		{m.ClusterContendedTables, "cluster_contended_tables", s.scrapeContendedTables},
		{m.ClusterContentionEvents, "cluster_contention_events", s.scrapeContentionEvents},
		{m.ClusterContendedKeys, "cluster_contended_keys", s.scrapeContendedKeys},
		{m.ClusterLocks, "cluster_locks", s.scrapeClusterLocks},
		{m.TransactionContentionEvents, "transaction_contention_events", s.scrapeTransactionContentionEvents},
		{m.Jobs, "jobs", s.scrapeJobs},
		{m.SchemaChanges, "schema_changes", s.scrapeSchemaChanges},
		{m.RangesNoLeases, "ranges_no_leases", s.scrapeRangeMetrics},
		{m.GossipLiveness, "gossip_liveness", s.scrapeGossipLiveness},
		{m.NodeMetrics, "node_metrics", s.scrapeNodeMetrics},
		{m.KVNodeStatus, "kv_node_status", s.scrapeKVNodeStatus},
	}
	...
	for _, sc := range scrapes {
		...
		if err := sc.fn(ctx, db, sm, now); err != nil {
			errorCount++
			s.logger.Error("Scrape failed",
				zap.String("metric_group", sc.name),
				zap.Error(err))
		}
	}

	s.recordReceiverMetrics(sm, now, errorCount)
	// Always return nil so partial scrape data flows downstream. Per-group
	// failures are logged above and counted in cockroachdb.receiver.scrape_errors.
	return metrics, nil
}
...
// Node Metrics. On virtual clusters (Serverless) crdb_internal.node_metrics
// only exposes (store_id, name, value) — node_id is unavailable, so we tag
// the resulting data points with store_id when present.
func (s *cockroachScraper) scrapeNodeMetrics(ctx context.Context, db *sql.DB, sm pmetric.ScopeMetrics, now pcommon.Timestamp) error {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&storeID, &name, &value); err != nil {
			s.logger.Error("Failed to scan node metrics row", zap.Error(err))
			continue
		}
		...
		setStore := func(dp pmetric.NumberDataPoint) {
			if storeID.Valid {
				dp.Attributes().PutStr("store_id", fmt.Sprintf("%d", storeID.Int64))
			}
		}
		switch name.String {
...
// Cluster Locks (active locks; may be unavailable on Serverless).
func (s *cockroachScraper) scrapeClusterLocks(ctx context.Context, db *sql.DB, sm pmetric.ScopeMetrics, now pcommon.Timestamp) error {
	ctx, cancel := context.WithTimeout(ctx, s.config.QueryTimeout)
	defer cancel()

	rows, err := db.QueryContext(ctx, queryClusterLocks, s.config.QueryLimit)
	if err != nil {
		return fmt.Errorf("failed to query cluster locks: %w", err)
	}
	defer rows.Close()

	lockCountMetric := sm.Metrics().AppendEmpty()
	lockCountMetric.SetName("cockroachdb.cluster.locks.active")
	lockCountMetric.SetUnit("{locks}")
	lockCountMetric.SetEmptyGauge()

	lockDurationMetric := sm.Metrics().AppendEmpty()
	lockDurationMetric.SetName("cockroachdb.cluster.locks.duration.max")
	lockDurationMetric.SetUnit("s")
	lockDurationMetric.SetEmptyGauge()

	for rows.Next() {
		var dbName, tableName, lockStrength sql.NullString
		var granted sql.NullBool
		var lockCount sql.NullInt64
		var maxDuration sql.NullFloat64

		if err := rows.Scan(&dbName, &tableName, &lockStrength, &granted, &lockCount, &maxDuration); err != nil {
			s.logger.Error("Failed to scan cluster locks row", zap.Error(err))
			continue
		}

		setAttrs := func(dp pmetric.NumberDataPoint) {
			if dbName.Valid {
				dp.Attributes().PutStr("database", dbName.String)
			}
			if tableName.Valid {
		...
		if lockCount.Valid {
			dp := lockCountMetric.Gauge().DataPoints().AppendEmpty()
			dp.SetTimestamp(now)
			dp.SetIntValue(lockCount.Int64)
			setAttrs(dp)
		}
		if maxDuration.Valid {
			dp := lockDurationMetric.Gauge().DataPoints().AppendEmpty()
			dp.SetTimestamp(now)
			dp.SetDoubleValue(maxDuration.Float64)
			setAttrs(dp)
		}
	}
...
// Cluster Contended Keys (key-level contention summaries).
func (s *cockroachScraper) scrapeContendedKeys(ctx context.Context, db *sql.DB, sm pmetric.ScopeMetrics, now pcommon.Timestamp) error {
	ctx, cancel := context.WithTimeout(ctx, s.config.QueryTimeout)
	defer cancel()

	rows, err := db.QueryContext(ctx, queryClusterContendedKeys, s.config.QueryLimit)
	if err != nil {
		return fmt.Errorf("failed to query cluster contended keys: %w", err)
	}
	defer rows.Close()

	contentionMetric := sm.Metrics().AppendEmpty()
	contentionMetric.SetName("cockroachdb.contention.key.events")
	contentionMetric.SetUnit("{events}")
	contentionMetric.SetEmptyGauge()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&dbName, &schemaName, &tableName, &indexName, &numEvents); err != nil {
			s.logger.Error("Failed to scan cluster contended keys row", zap.Error(err))
			continue
		}
		if !numEvents.Valid {
			continue
...
// Transaction Contention Events (per-event detail; expensive on busy clusters).
func (s *cockroachScraper) scrapeTransactionContentionEvents(ctx context.Context, db *sql.DB, sm pmetric.ScopeMetrics, now pcommon.Timestamp) error {
	ctx, cancel := context.WithTimeout(ctx, s.config.QueryTimeout)
	defer cancel()

	rows, err := db.QueryContext(ctx, queryTransactionContentionEvents, s.config.QueryLimit)
	if err != nil {
		return fmt.Errorf("failed to query transaction contention events: %w", err)
	}
	defer rows.Close()

	durationMetric := sm.Metrics().AppendEmpty()
	durationMetric.SetName("cockroachdb.contention.transaction.duration")
	durationMetric.SetUnit("s")
	durationMetric.SetEmptyGauge()

	for rows.Next() {
		var dbName, tableName, contentionType sql.NullString
		var durationSeconds sql.NullFloat64

		if err := rows.Scan(&dbName, &tableName, &contentionType, &durationSeconds); err != nil {
			s.logger.Error("Failed to scan transaction contention event row", zap.Error(err))
			continue
		}
		...
		dp := durationMetric.Gauge().DataPoints().AppendEmpty()
		dp.SetTimestamp(now)
		dp.SetDoubleValue(durationSeconds.Float64)
		if dbName.Valid {
			dp.Attributes().PutStr("database", dbName.String)
		}
		if tableName.Valid {
		...
		if contentionType.Valid {
			dp.Attributes().PutStr("contention_type", contentionType.String)
		}
	}
...
// Gossip Liveness — node liveness from KV layer. Unavailable on virtual
// clusters (Serverless): query returns "operation is unsupported within a
// virtual cluster". Failure is reported via the partial-error path.
func (s *cockroachScraper) scrapeGossipLiveness(ctx context.Context, db *sql.DB, sm pmetric.ScopeMetrics, now pcommon.Timestamp) error {
	ctx, cancel := context.WithTimeout(ctx, s.config.QueryTimeout)
	defer cancel()

	rows, err := db.QueryContext(ctx, queryGossipLiveness)
	if err != nil {
		return fmt.Errorf("failed to query gossip liveness: %w", err)
	}
	defer rows.Close()

	liveMetric := sm.Metrics().AppendEmpty()
	liveMetric.SetName("cockroachdb.node.live")
	liveMetric.SetUnit("{0|1}")
	liveMetric.SetEmptyGauge()

	for rows.Next() {
		var nodeID, isLive sql.NullInt64
		var membership sql.NullString
		if err := rows.Scan(&nodeID, &isLive, &membership); err != nil {
			s.logger.Error("Failed to scan gossip liveness row", zap.Error(err))
			continue
		}
		if !isLive.Valid {
			continue
		}
		dp := liveMetric.Gauge().DataPoints().AppendEmpty()
		dp.SetTimestamp(now)
		dp.SetIntValue(isLive.Int64)
		if nodeID.Valid {
			dp.Attributes().PutStr("node_id", fmt.Sprintf("%d", nodeID.Int64))
		}
		if membership.Valid {
			dp.Attributes().PutStr("membership", membership.String)
		}
	}
...
// KV Node Status — per-node network latency from the KV layer. Unavailable
// on virtual clusters (Serverless).
func (s *cockroachScraper) scrapeKVNodeStatus(ctx context.Context, db *sql.DB, sm pmetric.ScopeMetrics, now pcommon.Timestamp) error {
	ctx, cancel := context.WithTimeout(ctx, s.config.QueryTimeout)
	defer cancel()

	rows, err := db.QueryContext(ctx, queryKVNodeStatus, s.config.QueryLimit)
	if err != nil {
		return fmt.Errorf("failed to query kv node status: %w", err)
	}
	defer rows.Close()

	uptimeMetric := sm.Metrics().AppendEmpty()
	uptimeMetric.SetName("cockroachdb.node.uptime")
	uptimeMetric.SetUnit("s")
	uptimeMetric.SetEmptyGauge()

	lastUpdateMetric := sm.Metrics().AppendEmpty()
	lastUpdateMetric.SetName("cockroachdb.node.last_update.age")
	lastUpdateMetric.SetUnit("s")
	lastUpdateMetric.SetEmptyGauge()

	for rows.Next() {
		var nodeID sql.NullInt64
		var uptime, lastUpdate sql.NullFloat64
		if err := rows.Scan(&nodeID, &uptime, &lastUpdate); err != nil {
			s.logger.Error("Failed to scan kv node status row", zap.Error(err))
			continue
		}
		nodeAttr := func(dp pmetric.NumberDataPoint) {
			if nodeID.Valid {
				dp.Attributes().PutStr("node_id", fmt.Sprintf("%d", nodeID.Int64))
			}
		}
		if uptime.Valid {
			dp := uptimeMetric.Gauge().DataPoints().AppendEmpty()
			dp.SetTimestamp(now)
			dp.SetDoubleValue(uptime.Float64)
			nodeAttr(dp)
		}
		if lastUpdate.Valid {
			dp := lastUpdateMetric.Gauge().DataPoints().AppendEmpty()
			dp.SetTimestamp(now)
			dp.SetDoubleValue(lastUpdate.Float64)
			nodeAttr(dp)
		}
	}
